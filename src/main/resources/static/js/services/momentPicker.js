(function (angular) {
    ...
    var MomentPickerDirective = (function () {
        ...
        MomentPickerDirective.prototype.link = function ($scope, $element, $attrs) {
            ...
            // utilities
            $scope.momentToDate = function (value) {
                return $scope.isValidMoment(value) ? value.clone().toDate() : undefined;
            };
            $scope.valueUpdate = function () {
                if (!$scope.disabled) $scope.value = $scope.momentToDate($scope.valueMoment);
            };
            $scope.isValidMoment = function (value) {
                return angular.isDefined(value) && value.isValid();
            };
            $scope.limits = {
                ...
                isSelectable: function (value, precision) {
                    var selectable = true;
                    try {
                        if (angular.isFunction($scope.selectable)) selectable = $scope.selectable({
                            date: value,
                            type: precision
                        });
                    } catch (e) {
            ...
            $scope.view = {
                ...
                update: function () {
                    $scope.view.value = $scope.momentToDate($scope.view.moment);
                },
                toggle: function () {
            ...
            $scope.input = $scope.contents[0].tagName.toLowerCase() != 'input' && $scope.contents[0].querySelectorAll('input').length > 0 ? angular.element($scope.contents[0].querySelectorAll('input')) : angular.element($scope.contents[0]);
            ...
            $scope.input
                .on('focus click', function () {
                    $scope.$evalAsync($scope.view.open);
                })
                .on('blur', function () {
                    $scope.$evalAsync($scope.view.close);
                })
                .on('keydown', function (e) {
                    if ($scope.keyboard) $scope.$evalAsync(function () {
